 ...
 */
class TwitterAuthService extends Config
{
    ...
    {
        if (
            !Storage::exists($parameters['media']) ||
            ($file = Storage::get($parameters['media'])) === false
        ) {
    ...
    private function uploadMediaChunked(string $path, array $parameters)
    {
        ...
        $media = Storage::readStream($parameters['media'], 'rb');
        while (!feof($media)) {
            $this->http(
                'POST',
                self::UPLOAD_HOST,
        ...
        while(!$finished) {
            ...
            if (property_exists($finalize, 'processing_info')) {
                if ($finalize->processing_info->state === 'succeeded') {
                    $finished = true;
                } else {
                    // wait for again for some time before requesting again.
                    $timeToSleep = (int)$finalize->processing_info->check_after_secs + 5;

                    // for some reason twitter api is very slow in processing gif
                    // so we sleep for more time
                    if ($parameters['media_category'] === 'tweet_gif') {
                        $timeToSleep = 30;
                    }

                    sleep($timeToSleep);
                    Log::info("checking again if tweet is successful", (array)$finalize);
                }
            } else {
                $finished = true;
            }
    ...
    private function mediaInitParameters(array $parameters): array
    {
        ...
        $base = [
            'command' => 'INIT',
            'total_bytes' => Storage::size($parameters['media']),
        ];
